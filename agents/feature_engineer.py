...
# # from tools.feature_tools import preprocess_dataset
# # import json
# # import os
...
# #     print("\n⚙️  Agent 2: Feature Engineer is working...")
# #
...
# #     )
# #
# #     # Extract only summary — never send full data rows to LLM
...
# #
# # Summarize:
# # 1. What columns were dropped and why
# # 2. How missing values were handled
# # 3. What encoding was applied
# # 4. Final feature set ready for modelling
...
#     print("="*60)
#
#     df_original = pd.read_csv(state["file_path"])
#     print(f"  📥 Input shape           : {df_original.shape[0]} rows × {df_original.shape[1]} columns")
#
#     # Show what will be dropped
#     threshold = len(df_original) * 0.5
#     high_missing = [col for col in df_original.columns
#                     if df_original[col].isnull().sum() > threshold]
#     if high_missing:
#         print(f"\n  🗑️  Dropping high-missing columns (>50% missing):")
#         for col in high_missing:
#             pct = round(df_original[col].isnull().sum() / len(df_original) * 100, 1)
#             print(f"      • {col} ({pct}% missing)")
#
#     non_informative = [col for col in df_original.columns
#                        if df_original[col].nunique() == len(df_original)
#                        or df_original[col].nunique() == 1]
#     if non_informative:
#         print(f"\n  🗑️  Dropping non-informative columns:")
#         for col in non_informative:
#             print(f"      • {col} (unique values: {df_original[col].nunique()})")
#
#     # Show missing value strategy
#     print(f"\n  🔧 Missing value strategy:")
#     for col in df_original.columns:
#         missing = df_original[col].isnull().sum()
#         if missing > 0 and col not in high_missing:
#             strategy = "mode (categorical)" if df_original[col].dtype == "object" else "median (numeric)"
#             print(f"      • {col:<15} → fill with {strategy}")
#
#     # Show encoding
#     cat_cols = [col for col in df_original.select_dtypes(include="object").columns
#                 if col not in high_missing and col != state["target_column"]]
#     if cat_cols:
...
#     print(f"  ✅ Final features        : {full_result['final_columns']}")
#
#     prompt = f"""
# You are an expert feature engineer. Review the preprocessing steps taken.
#
